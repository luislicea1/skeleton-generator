...
import { parseSvgPrecise, parseSvgContour } from '../utils/svgParser';
import type { SvgData, SkeletonMode } from '../types/skeleton';
...
export default function Uploader({ mode, onModeChange, onLoad }: UploaderProps) {
  ...
  async function processFile(file: File) {
    ...
    if (file.type !== 'image/svg+xml' && !file.name.endsWith('.svg')) {
      setError(t.uploader.errorOnlySvg);
      return;
    }
  ...
  const reduction = sizeInfo
    ? Math.max(0, Math.round((1 - sizeInfo.skeleton / sizeInfo.original) * 100))
    : null;
  ...
  return (
    <div className="w-full flex flex-col gap-3">
        ...
        {processing ? (
          ...
          <div className="flex flex-col items-center justify-center gap-3 py-12 px-6">
            <div className={[
              ...
              <Icon
                icon="lucide:upload"
                className={['w-7 h-7', isDragging ? 'text-indigo-500 dark:text-indigo-400' : 'text-gray-400 dark:text-gray-500'].join(' ')}
              />
            </div>
      ...
      <input ref={inputRef} type="file" accept=".svg,image/svg+xml" className="hidden" onChange={handleChange} />
    </div>
  );
}
